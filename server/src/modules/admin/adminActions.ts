...
const countUsers: RequestHandler = async (req, res) => {
  try {
    const users = await adminRepository.countAll("user");

    if (users) {
      res.json(users.user_nb);
    } else {
      res.sendStatus(500);
    }
  } catch (err) {
...
export default { browseRecipes, countPending, countRecipes, countUsers };
